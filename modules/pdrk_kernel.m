...
for jpa = 1:npa % scan 1st parameter a
  ...
  for jpb = 1:npb % scan 2nd parameter b
    ...
    k=abs(par(1))/cwp; % k only >=0,¡¾m^-1¡¿
    theta=par(2); % the angle between k and B0,¡¾degree¡¿
    if(ipa>2 && ipb>2) % !! to update
      kz=par(3)/cwp; % ¡¾m^-1¡¿
      kx=par(4)/cwp; % ¡¾m^-1¡¿
    else
      kz=cos(theta*pi/180)*k; % ¡¾m^-1¡¿
      kx=sin(theta*pi/180)*k; % ¡¾m^-1¡¿
    end
    k=sqrt(kx*kx+kz*kz); % update k,¡¾m^-1¡¿
    theta=angle(kz+1i*kx)*180/pi; % update theta,¡¾degree¡¿
    ...
    if(sp==0) % solve all the roots, slow for large N
      ...
      if(iem==1 && iout==2 && icalp==1) % for pdrk_output.m
        ...
        %added by XYZhu 2020-05-27
        scaling_factor = 1.0;
        scaling(jpa,jpb,jpl) = scaling_factor*B0/dEB(5); %dBy/B0=0.1
        Pola_SI(jpa,jpb,jpl,1:6) = dEB*scaling(jpa,jpb,jpl);
        Pola_norm(jpa,jpb,jpl,1:6) = Pola_SI(jpa,jpb,jpl,1:6)/B0;
        Pola_norm(jpa,jpb,jpl,1:3) = Pola_norm(jpa,jpb,jpl,1:3)/vA;
        
        Pola_SI(jpa,jpb,jpl,7) = (Pola_SI(jpa,jpb,jpl,1)*conj(Pola_SI(jpa,jpb,jpl,1))+Pola_SI(jpa,jpb,jpl,2)*conj(Pola_SI(jpa,jpb,jpl,2))+Pola_SI(jpa,jpb,jpl,3)*conj(Pola_SI(jpa,jpb,jpl,3)))*epsilon0*0.5;
        Pola_SI(jpa,jpb,jpl,8) = (Pola_SI(jpa,jpb,jpl,4)*conj(Pola_SI(jpa,jpb,jpl,4))+Pola_SI(jpa,jpb,jpl,5)*conj(Pola_SI(jpa,jpb,jpl,5))+Pola_SI(jpa,jpb,jpl,6)*conj(Pola_SI(jpa,jpb,jpl,6)))/mu0*0.5;
        
        Pola_SI(jpa,jpb,jpl,9) = (c2*kz*Pola_SI(jpa,jpb,jpl,5)-D(1,1)*Pola_SI(jpa,jpb,jpl,1))*epsilon0/1i;
        Pola_SI(jpa,jpb,jpl,10) = (c2*kx*Pola_SI(jpa,jpb,jpl,6)-c2*kz*Pola_SI(jpa,jpb,jpl,4)-D(1,1)*Pola_SI(jpa,jpb,jpl,2))*epsilon0/1i;
        Pola_SI(jpa,jpb,jpl,11) = (-c2*kx*Pola_SI(jpa,jpb,jpl,5)-D(1,1)*Pola_SI(jpa,jpb,jpl,3))*epsilon0/1i;
        Pola_norm(jpa,jpb,jpl,9) = Pola_SI(jpa,jpb,jpl,9)/1i/(qs(1)*ns0(1)*vA);
        Pola_norm(jpa,jpb,jpl,10) = Pola_SI(jpa,jpb,jpl,10)/1i/(qs(1)*ns0(1)*vA);
        Pola_norm(jpa,jpb,jpl,11) = Pola_SI(jpa,jpb,jpl,11)/1i/(qs(1)*ns0(1)*vA);
        %added by Xingyu Zhu 2020-05-27
        run ../modules/pkues_velocity;
        %%%%%%%%%%%%%%%%
...
%             1i*sqrt(imag(dEB(1))^2+imag(dEB(2))^2+imag(dEB(3))^2);
        ...
        if idf==1 && jpa==jpa_df && jpb==jpb_df
            run ../modules/pkues_write_f_SI_unit;   
            disp([kx,kz,k]);
        end
