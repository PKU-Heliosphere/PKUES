% Coded by Xingyu Zhu and Jiansen He 2020-06-01
...
%%%%%%%%%%%%%%%%%%%%%%%
x = wws(jpa_input,jpb_input,jpl_input)*wcs1;
dEx = Pola_SI(jpa_input,jpb_input,jpl_input,1)/scaling_factor;
dEy = Pola_SI(jpa_input,jpb_input,jpl_input,2)/scaling_factor;
dEz = Pola_SI(jpa_input,jpb_input,jpl_input,3)/scaling_factor;
dBx = Pola_SI(jpa_input,jpb_input,jpl_input,4)/scaling_factor;
dBy = Pola_SI(jpa_input,jpb_input,jpl_input,5)/scaling_factor;
dBz = Pola_SI(jpa_input,jpb_input,jpl_input,6)/scaling_factor;
...
for timerun=0:num_periods*timesteps
    ...
    filename2=['pdrk_normEB_(kdi=' num2str(k*cwp,'%.3f') ...
        ')(ampl=' num2str(ampl,'%.2f') ')_' num2str(timerun,'%03d') '.vtk'];
    fileN=['/Users/psr/work/13. ÉÚÉù²¨/VDF data from pdrk/pdrk_SI_VDF_acsii_' num2str(timerun,'%03d') '.dat'];
    fileM=['/Users/psr/work/13. ÉÚÉù²¨/VDF data from pdrk/pdrk_SI_normEB_acsii_' num2str(timerun,'%03d') '.dat'];
...
%     title3 = 'dfqE';
%     title4 = 'dfqvB';
%     title3 = 'smooth_deltaf';
%     dfarray_s = smoothdata(dfarray,3,'movmedian',5);
%     farray_s = smoothdata(farray,3,'movmedian',5);
%     dfarray_s = farray_s-f0array;
    
%     if is_shift_df_phase==1 && timerun==0
%         phi_V_Dtensor = angle(dVnorm(jpa_input,1,s_input,jpl_input));
%         vx = VXarray(:,1,1);
%         vy = reshape(VYarray(1,:,1),1,[]);
%         vz = reshape(VZarray(1,1,:),1,[]);
%         ntotal = trapz(vz,trapz(vy,trapz(vx, f0array,3),2));
%         phi_V_df = angle(trapz(vx,trapz(vy,trapz(vz, VXarray.*dfarray,3),2))/ntotal);
%         dphi_tmp = phi_V_Dtensor-phi_V_df;
%     end
%     dfarraynew = real(complex(dfarray,imfarray).*exp(1i*dphi_tmp));
%     imfarraynew = imag(complex(dfarray,imfarray).*exp(1i*dphi_tmp));
%     dfarray=dfarraynew;
%     imfarray=imfarraynew;
    farray = f0array+dfarray;
...
%     filename = ['/Users/psr/theory analysis/PDRK_20200510/output/' filename];
%     filename2 = ['/Users/psr/theory analysis/PDRK_20200510/output/' filename2];
