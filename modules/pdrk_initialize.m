...
Qtotal=sum(qs0.*ns0); % charge density, ¡¾q/e?m^-3¡¿
Jtotal=sum(qs0.*ns0.*vds0); % current density, ¡¾q/e?m^-3?vds/c¡¿
...
vtzs=sqrt(2*kB*Tzs./ms); % para thermal velocity, note the sqrt(2),¡¾m/s¡¿
lambdaDs=sqrt(epsilon0*kB*Tzs./(ns0.*qs.^2)); % Debye length, Tzs,¡¾m¡¿
kDs=1./lambdaDs; % m^-1
wps=sqrt(ns0.*qs.^2./ms/epsilon0); % plasma frequency,¡¾rad/s¡¿
wcs=B0*qs./ms; % cyclotron frequency,¡¾rad/s¡¿
% rhocs=sqrt(kB*Tps./ms)./abs(wcs); % cyclotron radius
rhocs=sqrt(kB*Tps./ms)./wcs; % cyclotron radius,¡¾m¡¿, 2018-06-13 21:47

wps2=wps.^2; % square of plasma frequency,¡¾(rad/s)^2¡¿
lmdT=Tzs./Tps; % dimensionless

% for sigma=a,b due to the two perp temperatures for core and loss cone fv
Tpsab(1,:)=Tps; Tpsab(2,:)=alphas.*Tps; % two loss cone Tperps,¡¾K¡¿
lmdTab(1,:)=Tzs./Tpsab(1,:); lmdTab(2,:)=Tzs./Tpsab(2,:); % dimensionless
rhocsab(1,:)=sqrt(kB*Tpsab(1,:)./ms)./wcs; % first loss cone thermal radius,¡¾m¡¿
rhocsab(2,:)=sqrt(kB*Tpsab(2,:)./ms)./wcs; % second loss cone thermal radius,¡¾m¡¿

betasz=2*mu0*kB.*ns0.*Tzs./B0^2; % beta_para
betasp=2*mu0*kB.*ns0.*Tps./B0^2; % beta_perp
vA=B0/sqrt(mu0*sum(ms.*ns0)); % Total Alfven speed,¡¾m/s¡¿
vAs=B0./sqrt(mu0*ms.*ns0); % species Alfven speed,¡¾m/s¡¿
...
cSs1=sqrt(2*kB*Tzs(1)/ms(1)); % sound speed,¡¾m/s¡¿
wcs1=abs(wcs(1)); % omega_{ci},¡¾rad/s¡¿
wps1=sqrt(ns0(1)*qs(1)^2/ms(1)/epsilon0); % omega_{pi},¡¾rad/s¡¿
% thermal speed added by Xingyu Zhu 2020-06-23
vthzS = sqrt(2*kB*Tzs./ms); % thermal speed,¡¾m/s¡¿
vthpS = sqrt(2*kB*Tzs./ms); % thermal speed,¡¾m/s¡¿

cwp=sqrt(c2)/wps1; % c/omega_{p1},¡¾m¡¿
