...
k=sqrt(kz^2+kx^2); % k,¡¾m^-1¡¿
bsab=kx*rhocsab; % dimensionless
bsab(abs(bsab)<1e-50)=1e-50;  % to avoid singular when k_perp=0

bsab2=bsab.^2; % ¡¾dimensionless¡¿
M=sparse(NN,NN);
snj=0;
...
for s=1:S % species
  for n=-N:N % Bessel function
    ...
    for j=1:J % poles of Z(zeta)
      ...
      csnj(snj)=czj(j)*kz*vtzs(s)+kz*vds(s)+n*wcs(s); % ¡¾rad/s¡¿

      cnj=csnj(snj); % ¡¾rad/s¡¿
      bj0ab=vds(s)+(1-1/lmdTab(iab,s))*czj(j)*vtzs(s); % ¡¾m/s¡¿

      %
      if(n==0)  % for A_nj
        bnj1=bj0ab/(czj(j)*vtzs(s)+vds(s)); % avoid cnj=0, ¡¾dimensionless¡¿
      else
        bnj1=kz*bj0ab/cnj; % ¡¾dimensionless¡¿
      end
      bnj2=1-bnj1; % ¡¾dimensionless¡¿

      tmp=wps2(s)*bzj(j); % ¡¾(rad/s)^2¡¿

      b11snj(snj)=b11snj(snj)+rsab(iab,s)*tmp*bnj2*n^2*Gamn/bsab2(iab,s); % ¡¾(rad/s)^2¡¿
      b11=b11+rsab(iab,s)*tmp*bnj1*n^2*Gamn/bsab2(iab,s); % ¡¾(rad/s)^2¡¿

      b12snj(snj)=b12snj(snj)+rsab(iab,s)*tmp*bnj2*1i*n*Gamnp; % ¡¾(rad/s)^2¡¿
      b12=b12+rsab(iab,s)*tmp*bnj1*1i*n*Gamnp; % ¡¾(rad/s)^2¡¿
      b21snj(snj)=-b12snj(snj); % ¡¾(rad/s)^2¡¿
      b21=-b12; % ¡¾(rad/s)^2¡¿

      b22snj(snj)=b22snj(snj)+rsab(iab,s)*tmp*bnj2*(n^2*Gamn/bsab2(iab,s)...
          -2*bsab2(iab,s)*Gamnp); % ¡¾(rad/s)^2¡¿
      ...
      if(n==0)  % for eta_n*A_nj
        ...
        bnj1=n*wcs(s)*bj0ab/cnj/vtzs(s); % ¡¾dimensionless¡¿
      end
      bnj2=czj(j)/lmdTab(iab,s)+bnj1; % ¡¾dimensionless¡¿

      b13snj(snj)=b13snj(snj)+rsab(iab,s)*tmp*bnj2*n*...
          sqrt(2*lmdTab(iab,s))*Gamn/bsab(iab,s); % ¡¾(rad/s)^2¡¿
      b13=b13-rsab(iab,s)*tmp*bnj1*n*sqrt(2*lmdTab(iab,s))*Gamn/bsab(iab,s); % ¡¾(rad/s)^2¡¿
      b31snj(snj)=b13snj(snj); % ¡¾(rad/s)^2¡¿
      b31=b13; % ¡¾(rad/s)^2¡¿

      b23snj(snj)=b23snj(snj)-rsab(iab,s)*1i*tmp*bnj2*...
          sqrt(2*lmdTab(iab,s))*Gamnp*bsab(iab,s); % ¡¾(rad/s)^2¡¿
      b23=b23+rsab(iab,s)*1i*tmp*bnj1*sqrt(2*lmdTab(iab,s))*Gamnp*bsab(iab,s); %
      b32snj(snj)=-b23snj(snj); % ¡¾(rad/s)^2¡¿
      b32=-b23; % ¡¾(rad/s)^2¡¿

      %
      if(bj0ab==0 || kz==0)  % for eta_n^2*A_nj
        bnj1=0; % ¡¾dimensionless¡¿
        bnj2=czj(j)*czj(j); % ¡¾dimensionless¡¿
      else
        ...
        bnj2=(vds(s)/vtzs(s)+czj(j))*czj(j)/lmdTab(iab,s)+...
          n*wcs(s)*bj0ab*(1-n*wcs(s)/cnj)/vtzs(s)^2/kz; % ¡¾dimensionless¡¿
      end

      b33snj(snj)=b33snj(snj)+rsab(iab,s)*tmp*bnj2*2*lmdTab(iab,s)*Gamn; % ¡¾(rad/s)^2¡¿
      b33=b33+rsab(iab,s)*tmp*bnj1*2*lmdTab(iab,s)*Gamn; % ¡¾(rad/s)^2¡¿
      end
    end
  end
end
